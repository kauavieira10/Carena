...
window.App = {
  ...
  async boot(){
    this.wire();
    // 1) Desenha JÁ com os dados embutidos da planilha (garante que sempre aparece algo)
    this.FULL = (window.DATASET || []).slice();
    this.goals = window.GOALS_FALLBACK || CONFIG.goals;
    this.monthDays = this.FULL.length || CONFIG.monthDaysFallback;
    this.data = this.FULL.slice();
    this.render();
    console.log('[App] Dados da planilha exibidos · '+this.FULL.length+' dias');
    // 2) Em segundo plano, tenta atualizar com dados AO VIVO (só funciona com backend/deploy)
    try {
      const out = await Sheets.fetchData();
      this.FULL = out.dias; this.goals = out.goals; this.monthDays = out.dias.length || this.monthDays;
      this.data = this.FULL.slice();
      this.render();
      this.toast('Dados atualizados da planilha (ao vivo)', 'online');
    } catch (e) {
      console.warn('[Sheets] sem conexao ao vivo (usando dados embutidos):', e.message);
      this.toast('Dados da planilha — mai/2026', 'demo');
    }
  }
};
